package com.app.dialy_cafe.exception.exceptionValidateInput;

import com.app.dialy_cafe.exception.constand.FieldBlankExceptionHandler;
...
public class Validation {
    public void ValidationPhoneNumber(String phoneNumber) {
        ...
        if (!phoneNumberMatcher.matches()) {
            throw new FieldBlankExceptionHandler(
                    "Phone number must be start with 0 and not allow to input character, special character or more than 11 digits. ");
        }
    }
    ...
    public void ValidationDate(String inputDate) {
        ...
        if (verifydate == 0) {
            if (month == 2 && (day > 29 || day <= 0)) {
                throw new FieldBlankExceptionHandler(("Day is not correct, because February, " + year
                        + ", has only 29 days.Example format yyyy-mm-dd"));
            }
        } else {
            if (month == 2 && (day > 28 || day <= 0)) {
                throw new FieldBlankExceptionHandler(("Day is not correct, because February, " + year
                        + ", has only 28 days.Example format yyyy-mm-dd"));
            }
        }
        ...
        if (month <= 7 && month > 0) {
            ...
            if (dayEarchMonth == 0) {
                if (day <= 0 || day > 30) {
                    throw new FieldBlankExceptionHandler(("Day is not correct, because " + monthWithString + ", " + year
                            + ", hss only 30 days.Example format yyyy-mm-dd"));
                }
            } else {
                if (day <= 0 || day > 31) {
                    throw new FieldBlankExceptionHandler(("Day is not correct, because " + monthWithString + ", " + year
                            + ", has only 31 days.Example format yyyy-mm-dd"));
                }
            }
        }
        if (month > 7) {
            ...
            if (dayEarchMonth == 0) {
                ...
                if (day <= 0 || day > 30) {
                    throw new FieldBlankExceptionHandler(("Day is not correct, because " + monthWithString + ", " + year
                            + ", has only 30 days.Example format yyyy-mm-dd"));
                }
            }
        }
        if (month >= 13 || month <= 0) {
            throw new FieldBlankExceptionHandler(
                    ("Month is not correct,because month of year have only 12 month so can input less then 13 and more then 0.Example format yyyy-mm-dd"));
        }
        if (lengthofyear >= 5) {
    ...
    public void ValidationDatePost(String inputDate) {
        ...
        if (verifydate == 0) {
            if (month == 2 && (day > 29 || day <= 0)) {
                throw new FieldBlankExceptionHandler(
                        ("Day is not correct, because February, " + year + ", has only 29 days."));
            }
        } else {
            if (month == 2 && (day > 28 || day <= 0)) {
                throw new FieldBlankExceptionHandler(
                        ("Day is not correct, because February, " + year + ", has only 28 days."));
            }
        }
        if (month <= 7 && month > 0) {
            dayEarchMonth = month % 2;
            if (dayEarchMonth == 0) {
                if (day <= 0 || day > 30) {
                    throw new FieldBlankExceptionHandler(
                            ("Day is not correct, because " + monthWithString + ", " + year + ", hss only 30 days"));
                }
            } else {
                if (day <= 0 || day > 31) {
                    throw new FieldBlankExceptionHandler(
                            ("Day is not correct, because " + monthWithString + ", " + year + ", has only 31 days"));
                }
            }
        }
        if (month > 7) {
            ...
            if (dayEarchMonth == 0) {
                ...
                if (day <= 0 || day > 30) {
                    throw new FieldBlankExceptionHandler(
                            ("Day is not correct, because " + monthWithString + ", " + year + ", has only 30 days"));
                }
            }
        }
        if (month >= 13 || month <= 0) {
            throw new FieldBlankExceptionHandler(
                    ("Month is not correct,because month of year have only 12 month so can input less then 13 and more then 0"));
        }
        if (lengthofyear >= 5) {
            throw new FieldBlankExceptionHandler(("Year is not correct,because allow input only less then 5 digit."));
        }
    ...
    public void ValidationEmail(String inputEmail) {
        ...
        if (!matcher1.matches()) {
            throw new FieldBlankExceptionHandler(
                    "🚫 Oops! Your email seems incorrect. Please use the format: example@gmail.com 🌈");
        }
    }
    ...
    public void ValidationPassword(String inputPassword) {
        ...
        if (!matcher2.matches()) {
            throw new FieldBlankExceptionHandler(
                    ("Password must be more then 8 digit with characters, number and special characters!"));
        }
    }
    ...
    public void ValidationInputOnlyText(String inputText, String feil) {
        ...
        if (!matcher2.matches() || !(inputText.equals("Male") || inputText.equals("Female"))) {
            throw new FieldBlankExceptionHandler((feil
                    + "is can not allow input with special character or number, Allow input only Female and Male"));
        }
    }
    ...
    public void validatePhoneNumber(String phoneNumber) {
        ...
        if (!phoneNumberMatcher.matches()) {
            throw new FieldBlankExceptionHandler(
                    "Phone number must start with 0 and cannot contain special characters or more than 11 digits.");
        }
    }
    ...
    // Method to validate LocalDateTime and convert it to ZonedDateTime for Phnom
    // Penh time zone
    public void validationDateWithLocalDateTime(LocalDateTime inputDateTime) {
        try {
            ...
            // Check if the input date-time is in the future compared to Phnom Penh current
            // time
